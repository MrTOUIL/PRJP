...
router.post("/login",
  ...
  async (req, res) => {
    ...
    try {
      ...
      // Determine role: some collections (admins) don't store a `role` field
      let role = user.role;
      if (!role) {
        // double-check admins collection explicitly
        const maybeAdmin = await admins.findOne({ email });
        if (maybeAdmin) role = 'admin';
      }

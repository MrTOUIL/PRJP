...
export default function ManageUsers() {
  ...
  const sendWarning = () => {
    ...
    apiJson(`/api/admin/warn-member/${selectedMember.id}`, {
      method: 'POST',
      body: { message: warnMessage.trim(), adminId },
    })
      .then(() => {
  ...
  const renderItem = ({ item }: { item: Member }) => (
    ...
    >
      <TouchableOpacity
        onPress={() => router.push({ pathname: '/(admin)/member/[id]', params: { id: item.id } })}
        style={styles.userInfo}
        activeOpacity={0.8}
      >
        <View style={[styles.avatar, { backgroundColor: isDark ? '#1F2937' : '#E2E8F0' }]}>

...
export default function ManagePosts() {
  ...
  const handleSearch = async (customQuery?: string) => {
    ...
    try {
      ...
      const mappedDevis = devis.map((item: any) => ({
        ...
        owner: item.email || item.owner || 'Unknown',
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      await apiJson(endpoint, { method: 'DELETE', body: { adminId } });
  ...
  const renderItem = ({ item }: { item: AdminItem }) => (
    <View style={[styles.card, { backgroundColor: isDark ? '#101827' : '#FFFFFF' }]}>
      <TouchableOpacity
        onPress={() => router.push({ pathname: '/(admin)/post/[id]', params: { id: item.id, type: item.type } })}
        activeOpacity={0.85}
      ...
      >
        ...
        <ThemedText style={styles.meta}>Owner: {item.owner}</ThemedText>
        ...
        <View style={styles.logsSection}>
          ...
          {logs.length === 0 ? (
            ...
            logs.slice(0, 5).map(log => (
              <View key={log.id} style={[styles.logRow, { backgroundColor: isDark ? AdminTheme.colors.surfaceElevated : '#F8FAFF' }]}>

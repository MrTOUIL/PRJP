...
export default function SeeAllStudents() {
	...
	useEffect(() => {
		const fetchAllStudents = async (): Promise<void> => {
			try {
				...
				const requestAllStudents = (token: string | null) => {
					return fetch(`${BASE_URL}/teacher/getallstudents`, {
						method: 'GET',
						headers: {
				...
				requestAllStudents(accessToken)
					.then(res => res.json())
					.then(data => {
						if (data.succ && Array.isArray(data.students) && Array.isArray(data.parents)) {
							...
							if (safeStudents.length === 0 && safeParents.length === 0) {
								setMessage('No students or parents assigned yet.');
							}
							...
							fetch(`${BASE_URL}/teacher/refresh`, {
								...
								.then(refreshData => {
									if (refreshData.accessToken) {
										...
										return requestAllStudents(refreshData.accessToken)
											.then(res => res.json())
											.then(retryData => {
			...
			<Animated.View key={`${type}-${safeItem._id ?? index}`} entering={FadeInDown.delay(index * 80).springify()}>
				<View style={styles.userCard}>
					...
					<View style={styles.userContent}>
						...
						{/*add the name of service */}
						{safeItem.service?.name && (
							<Text style={styles.userMeta}>Service: {safeItem.service.name}</Text>
						)}
		...
		<View style={styles.container}>
			...
			<View style={styles.header}>
				...
				<View style={styles.headerTextWrap}>
					<Text style={styles.headerTitle}>All My Students</Text>
					<Text style={styles.headerSubtitle}>Students and parents across all services</Text>
				</View>
			</View>

			<ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
				<Animated.View entering={FadeInDown.delay(80).springify()} style={styles.infoCard}>
					<Text style={styles.infoTitle}>All assigned users</Text>
					<Text style={styles.infoText}>This page groups every student and parent linked to your services.</Text>
				</Animated.View>
